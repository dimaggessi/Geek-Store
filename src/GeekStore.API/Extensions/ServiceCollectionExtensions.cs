    ...
    public static class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddSwaggerConfiguration(this IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                // Adiciona tags manualmente aos controladores, se necess�rio
                options.DocInclusionPredicate((docName, apiDesc) =>
                {
                    return true; // Incluir todas as rotas no mesmo documento (v1)
                });

                // Adicionar descri��es nas tags, como "Authorization" para as rotas de autentica��o
                options.TagActionsBy(apiDesc =>
                {
                    if (apiDesc.RelativePath?.StartsWith("api/auth") == true)
                    {
                        return new[] { "Authorization" }; // Atribuir tag "Authorization" aos endpoints de autentica��o
                    }
