...
@Component({
  ...
  imports: [ReactiveFormsModule, CommonModule, NgbModalModule, ErrorModalComponent],
})
export class RegisterComponent implements OnInit, AfterViewInit {
  ...
  get getFirstName(): string {
    ...
    return control?.touched && control?.invalid ? 'O nome é obrigatório.' : 'Nome';
  }

  get getLastName(): string {
    const control = this.f.lastName;
    return control?.touched && control?.invalid ? 'O sobrenome é obrigatório.' : 'Sobrenome';
  }

  get getEmail(): string {
    const control = this.f.email;
    return control?.touched && control?.invalid ? 'O email é obrigatório.' : 'Email';
  }

  get getPassword(): string {
    const control = this.f.password;
    return control?.touched && control?.invalid ? 'A senha é obrigatória.' : 'Senha';
  }
}
